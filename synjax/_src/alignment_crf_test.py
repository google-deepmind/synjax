...
"""Tests for alignment_crf."""

from absl.testing import absltest
import jax
import jax.numpy as jnp
from synjax._src import alignment_crf
...
class AlignmentCrfTest(distribution_test.DistributionTest):
  ...
  def create_random_batched_dists(self, key: jax.random.KeyArray):
    ...
    dists = [alignment_crf.AlignmentCRF((step_0, step_1), step_0),
             alignment_crf.AlignmentCRF((step_0, step_1), None)]
    return dists
  ...
  def test_crash_on_invalid_shapes(self):
    ...
    self.assertRaises(
        e, lambda: alignment_crf.AlignmentCRF((step_0, step_1), step_0))
    self.assertRaises(
        e, lambda: alignment_crf.AlignmentCRF((step_0, step_1), None))
    self.assertRaises(
        e, lambda: alignment_crf.AlignmentCRF((step_0,), step_1))
